    ...
    public static SentenceData parseWords(SentenceData sentenceData, int blankSymbolId){
        ...
        for (int i = 0; i < sentenceData.getParsedSentenceWordsList().size(); i++){
            try {
                ...
                sentenceData.getParsedSentenceWordsList().get(i).getPhonemes().add(blankSymbolId); //Add a blank symbol to the beginning of the word//TODO Make this a reference.
                while(rs.next()){
                    symbolIdList2.add(rs.getInt(4));
                    sentenceData.getParsedSentenceWordsList().get(i).getPhonemes().add(rs.getInt(4));
                }
                //sentenceData.getParsedSentenceWordsList().get(i).getPhonemes().add(blankSymbolId); //Add symbol for a closed mouth to the end of the word. //TODO Make this a reference.
